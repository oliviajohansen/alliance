...
import { BuddyUpIndustryData } from "../data";
...
const Buddy_up_industry = ({ navigation }) => {
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>Your industry matched with:</Text>
      <FlatList
        data={BuddyUpIndustryData.filter((b) => b.id == 1 || b.id == 2)}
      ...
      />
      <Text style={styles.title}>Similar Industry:</Text>
      <FlatList
        data={BuddyUpIndustryData.filter((b) => b.id != 1 && b.id != 2)}
        keyExtractor={(item) => item.id}
        ItemSeparatorComponent={() => <Divider />}
